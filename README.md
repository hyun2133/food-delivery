...
4. 고객이 주문을 취소할 수 있다.
5. 주문이 취소되면 배달이 취소된다.
6 .주문 내역이 rider에게 공유 된다.
7. 상점주인이 주문 내역 확인하여 승인 또는 거절한다.
8. 상점주인이 조리를 시작한다.
9. 상점주인이 조리를 종료하고 rider 을 호출한다..
10. rider가 음식을 pick 해서 배달을 시작한다. 
11. 고객이 음식을 받는다.
12. 고객이 주문상태를 중간중간 조회한다
13. 주문상태가 바뀔 때 마다 카톡으로 알림을 보낸다.
14. 고객은 리뷰를 올린다.
15. 상점주인이 리뷰에 댓글을 올린다.
